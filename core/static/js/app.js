...
const createPreloader = () => {
  ...
  centerDot.style.backgroundColor = "#00aeffff";
  ...
  for (let r = 0; r < rings; r++) {
    ...
    const numDots = 6 + r * 6;
// Calculate color based on ring (blue inner to white outer)
const blueAmount = Math.max(0, 1 - r / (rings - 1));
const colorR = Math.floor(255 * (1 - blueAmount)); // 0 → 255
const colorG = Math.floor(255 * (1 - blueAmount)); // 0 → 255
const colorB = 255; // Always 255 for blue → white
